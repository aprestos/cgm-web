...
<template>
  <!--
    Client-only, because which of the two branches below is correct depends on
    the viewport and a server has none. `useMinWidth` answers false with no
    browser, so the server rendered the slide-over and a desktop browser
    replaced it with the pinned `<aside>` — a hydration mismatch on every page
    that uses this, and the reason the two layouts are matched in JavaScript
    rather than toggled with CSS in the first place.

    No fallback on purpose: the panel is a control, not content, and guessing a
    viewport server-side only trades a mismatch for a flash of the wrong
    layout.
  -->
  <ClientOnly>
